...
export class ArchiveComponent implements OnInit {
  ...
  SetImages(articleData) {
    ...
    for (let index = 0; index < articleData.length; index++) {
      this.articleService.GetArticleImg(articleData[index].featured_media).subscribe(_featured_media => {
  ...
  fetchArticles(type, param1) {
    ...
    if (type == 'GetArticles') {
      ...
      this.articleService.GetArticlesByCategory(param1).subscribe(articleData => {
        this.SetImages(articleData);
      });
    }
  }
    ...
    this.articleService.GetAllCategories().subscribe(catdata => {
      ...
      this.route.queryParams.subscribe(params => {
        ...
        if (Author !== undefined) {
          ...
          if (Author == this.user.userNameAuth) { this.showSubscribeBtn = false; }
          else { this.showSubscribeBtn = true; }
          ...
          let cat = this.categories.filter(x => { return x.id == Category })[0];
          ...
          this.articleService.GetArticlesSearchResults(SearchTerm).subscribe(data => {
            this.sortedArticles = data;
          });
          this.resultsNumberFor = `we found ${this.sortedArticles.length} results for '${SearchTerm}'`;
          ...
          this.resultsNumberFor = `here ${this.sortedArticles.length} articles with tag '${Tag}'`;
          this.searchResLbl = ``;
          this.searchTerm = '';
        }
        else {
